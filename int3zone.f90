...
! extrapolation, eps-based acceptance) is shared with int1zone/int2zone via
! bsstep_mod -- see its header. What's specific to this routine is the
! trapezoidal sub-step physics (reset3/substep3 below), which extrapolates
! the zero-based core/middle transfer+coupling, middle/envelope
! transfer+coupling, and wind-loss sums (sumtrancm, sumtranme, sumdj) over
! the step, and reconstructs sjcore1/sjc1/sje1 from those sums only after
! extrapolation accepts. Track quantities are obtained at intermediate
! times by cubic-spline interpolation between points j-1 and j, using
! coefficients (yicore, yic, yie, ycm, ycz, ystr, ytau, ycen) precomputed
! by splinc in the caller.
!
...
! gets the tighter (not the looser) tolerance. (The original routine
! recomputed this every retry from the saved start-of-step angular
! momenta, which are restored unchanged before each retry -- so it was
! always the same value; computed once here instead, with no change in
! behavior.)
!
! Inputs:
...
subroutine int3zone(sage, sicore, sic, sie, staucz, fcm, fcz, fstruct, &
    ...
    ! state shared between reset3/substep3 below via host association --
    ! see int2zone for the analogous two-zone layout. djcoupcm0/1,
    ! djcoupme0/1 persist across the whole call (not reset per retry),
    ! exactly like int2zone's djcoup0/1.
    real(8) :: hh
    real(8) :: sicore0v, sicore1v, sic0v, sic1v, sie0v, sie1v
    real(8) :: sstr0, scen0
    real(8) :: srcm0, srcmmin, srcmmax
    real(8) :: srcz0, srczmin, srczmax
    real(8) :: stau0, staumin, staumax
    real(8) :: swe0, swc0, swcore0
    real(8) :: w0, wc0
    real(8) :: djcoupcm0, djcoupcm1, djcoupme0, djcoupme1
    real(8) :: sjcoresav, sjcsav, sjesav
    real(8) :: tcouplecm0, tcouple0
    real(8) :: wtest
    real(8), dimension(3) :: y0v, yscalv, y1v
    logical :: converged, lcmlin
    real(8) :: srcmlin
    ...
    yscalv(3) = fstruct(j-1)*wtest*min(w0,wcrit)**(exw-1.0d0)
    ! error scales for the two transfer+coupling sums: scale against the
    ! smaller of the two reservoirs each boundary moves angular momentum
    ! between (see header note on error scaling). right when the core
    ! first forms, sjcore0 is exactly zero by construction (see
    ! threezoneevol) -- there is no existing core budget to protect yet,
    ! so fall back to scaling against the middle zone's budget alone
    ! rather than dividing the error estimate by zero.
    if (sjcore0.gt.0.0d0) then
        yscalv(1) = min(sjcore0, sjc0)
    else
        yscalv(1) = sjc0
    endif
    yscalv(2) = min(sjc0, sje0)
    djcoupcm0 = 0.0d0
    djcoupcm1 = 0.0d0
    djcoupme0 = 0.0d0
    djcoupme1 = 0.0d0
    ...
    y0v = 0.0d0
    call bs_extrapolate(reset3, substep3, 3, dtt, t0, y0v, yscalv, y1v, converged)

    if (converged) then
        sjcore0 = sjcoresav
        sjc0 = sjcsav
        sje0 = sjesav
        sjcore1 = sjcore0 + y1v(1)
        sje1 = sje0 + y1v(2) + y1v(3)
        sjc1 = sjc0 - y1v(1) - y1v(2)
        lok = .true.
        return
    endif
    ...
    ! core formation on a real track. (Only reachable when substep3 never
    ! set iermsg itself, i.e. every retry ran to completion but none
    ! converged -- if substep3 aborted early, iermsg is already set and
    ! this must not clobber it.)
    if (iermsg.eq.0) iermsg = 5299
    lok = .false.
    return
...
contains
    ...
    ! this retry, restoring sjcore0/sjc0/sje0 from the saved start-of-step
    ! values first. See int2zone's reset2 for why djcoupcm0/1, djcoupme0/1
    ! are deliberately not reset here.
    subroutine reset3(ok)
    ...
    end subroutine reset3

    ! advance sje0/sjc0/sjcore0 (and swe0/swc0/swcore0/...) from t1-h1 to
    ! t1 by one trapezoidal sub-step, accumulating the core/middle
    ! transfer+coupling sum into y(1), the middle/envelope transfer+
    ! coupling sum into y(2), and the wind-loss sum into y(3) -- exactly
    ! as the original inline loop body.
    subroutine substep3(h1, t1, y, ok)
        real(8), intent(in) :: h1, t1
        real(8), intent(inout) :: y(:)
        logical, intent(out) :: ok
        real(8) :: a, b
        real(8) :: sstr1, scen1, srcm1, srcz1, stau1, tcouplecm1, tcouple1
        real(8) :: fcc0, fcc1, fc0, fc1
        real(8) :: djdt0, djdt1
        real(8) :: djtrancm0, djtrancm1, djtranme0, djtranme1
        real(8) :: swe1t, swc1t, swcore1t
        real(8) :: swe1, swc1, swcore1
        real(8) :: w1, wc1
        ...
        if (2.0d0*cpi/swe1/staucz(j-1) .gt. rocrit*rosun) then
            ...
            y(3) = y(3) + 0.5d0*h1*(djdt0+djdt1)
        endif
        y(1) = y(1) + 0.5d0*(djtrancm0+djtrancm1)+ &
               0.5d0*h1*(djcoupcm0+djcoupcm1)
        y(2) = y(2) + 0.5d0*(djtranme0+djtranme1)+ &
        ...
        sjc0 = sjc1
        sjcore0 = sjcore1
    end subroutine substep3

end subroutine int3zone
